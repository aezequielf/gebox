...
from .utils.semana import dia_num2let, genera_semana, dia_num2ab
# Create your views here.


#funcion para generar agenda 
def generar_turnos_semanal():
...
def index_agenda(request):
    if request.method == 'GET':
        ...
        if len(diario) == 0:
            #generar agenda
            generar_turnos_semanal()
            diario = turnos.objects.filter( dia_hora__gte = dia_inicio_semana).order_by('dia_hora')
            #comentar todas las lineas para que llegue al render final
...
from datetime import datetime, timedelta
from .models import turnos, grupos, params_ag
from .utils.semana import dia_num2let, genera_semana
# Create your views here.


#funcion para generar agenda 
def generar_turno_semanal():
        parametros = params_ag.objects.all()
        # creo un set para obtener días no repetidos desde los parametros
        param_dias = set()
        for param in parametros:
